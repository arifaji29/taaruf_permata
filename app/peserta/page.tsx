    ...
    <main className="min-h-screen bg-slate-50 pb-32 relative overflow-x-hidden">
      ...
      <div className="px-4 py-6 max-w-6xl mx-auto">
        <header className="mb-6">
          <h1 className="text-3l font-black text-slate-800 tracking-tighter uppercase">Temukan Jodohmu Di sini</h1>
          <p className="text-[10px] text-emerald-600 font-bold uppercase tracking-widest opacity-70">Database Peserta Permata</p>
        </header>

        {/* Filter Pill Section - Ukuran lebih kecil */}
        <div className="flex bg-white/50 backdrop-blur-md p-1 rounded-full border border-white/80 shadow-sm w-fit mb-8">
        ...
        {!peserta || peserta.length === 0 ? (
          <div className="p-16 border-2 border-dashed border-emerald-200 rounded-3xl text-center bg-white/30 backdrop-blur-sm">
            <p className="text-emerald-800 text-sm font-bold italic">Belum ada peserta yang mempublikasikan biodata.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {peserta.map((p: Peserta) => (
              <div key={p.id} className="bg-white rounded-4xl overflow-hidden shadow-lg shadow-slate-200/50 border border-slate-100 flex flex-col transition-transform active:scale-[0.98]">
                ...
                {/* Bagian Foto - Tinggi dikurangi agar kompak */}
                ...
                <div className="p-4 flex flex-col flex-1">
                  ...
                  {/* Info Statistik - Ukuran teks & padding dikecilkan */}
                  ...
                  {/* Tombol Detail - Pengganti Hover agar Mobile Friendly */}
                  <Link 
                    href={`/peserta/${p.id}`} 
