...
export default function ManajemenTim() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Simpan referensi form ke variabel agar tidak null setelah await
    ...
    if (error) {
      ...
      formElement.reset() // Menggunakan referensi variabel yang aman
      fetchTim()
    }
    setLoading(false) // Loading sekarang akan berhenti dengan benar
  }
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 h-fit">
            <h2 className="text-sm font-black mb-4 border-b pb-2 uppercase text-slate-800 flex items-center gap-2">
              <UserPlus size={16} className="text-emerald-600" /> Tambah Petugas
            </h2>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="flex flex-col">
                ...
                <input name="nama" className="p-2.5 text-xs border border-gray-200 rounded-xl outline-none focus:ring-2 focus:ring-emerald-500 bg-gray-50/50 text-slate-900 font-bold" required />
              </div>
              ...
              <div className="flex flex-col">
                ...
                <select name="dapukan" className="p-2.5 text-xs border border-gray-200 rounded-xl outline-none focus:ring-2 focus:ring-emerald-500 bg-white text-slate-900 font-bold cursor-pointer" required>
              ...
              <div className="flex flex-col">
                ...
                <input name="nomor_telepon" placeholder="628..." className="p-2.5 text-xs border border-gray-200 rounded-xl outline-none focus:ring-2 focus:ring-emerald-500 bg-gray-50/50 text-slate-900 font-bold" required />
              </div>
              ...
              <div className="flex flex-col">
                ...
                <textarea name="alamat_lengkap" rows={2} className="p-2.5 text-xs border border-gray-200 rounded-xl outline-none focus:ring-2 focus:ring-emerald-500 bg-gray-50/50 resize-none text-slate-900 font-bold" required />
              </div>

              <button type="submit" disabled={loading} className="w-full bg-emerald-600 text-white py-3 rounded-xl text-[10px] font-black uppercase tracking-widest shadow-md hover:bg-emerald-700 transition active:scale-95 disabled:opacity-50 flex items-center justify-center gap-2">
                {loading ? <div className="animate-spin rounded-full h-3 w-3 border-2 border-white/20 border-t-white"></div> : <UserPlus size={14} />}
                Daftarkan Petugas
              </button>
            </form>
          </div>
